...
if(isset($_SESSION['ct_details']) && $_SESSION['ct_details']!=''){
    ...
    /** check and add booking for guest customer **/
     if($_SESSION['ct_details']['guest_user_status'] == 'on'){
        ...
        if($_SESSION['ct_details']['payment_method'] == ''){
            ...
            $payment->order_id =$orderid;
            $payment->payment_method=ucwords($_SESSION['ct_details']['payment_method']);
            if(isset($_POST['transaction_id'])){
				$payment->transaction_id= $_POST['transaction_id'];
			} else if(isset($_SESSION['ct_details']['stripe_trans_id']) && $_SESSION['ct_details']['payment_method'] == 'stripe-payment'){
			$payment->transaction_id= $_SESSION['ct_details']['stripe_trans_id'];
			} else if(isset($_SESSION['ct_details']['twocheckout_trans_id']) && $_SESSION['ct_details']['payment_method'] == '2checkout-payment'){
			$payment->transaction_id= $_SESSION['ct_details']['twocheckout_trans_id'];
			} else if(isset($_SESSION['ct_details']['paumoney_transaction_id']) && $_SESSION['ct_details']['payment_method'] == 'payumoney'){
			$payment->transaction_id= $_SESSION['ct_details']['paumoney_transaction_id'];
			}	else if(isset($_SESSION['ct_details']['paumoney_transaction_id']) && $_SESSION['ct_details']['payment_method'] == 'payumoney2'){
			$payment->transaction_id= $_SESSION['ct_details']['paumoney_transaction_id'];
			}else {
				$payment->transaction_id='';
			}
        ...
        $user->existing_password=md5($_SESSION['ct_details']['existing_password']);
        ...
        if($existing_login){
            ...
            $_SESSION['login_user_id'] = $inserted_user;
            $_SESSION['useremail'] = $_SESSION['ct_details']['email'];
    ...
    for($i=0;$i<(count($_SESSION['ct_cart']['method']));$i++){
        ...
        while($jj = mysqli_fetch_array($hh)){
            ...
            else
            {
                ...
                $detectives = $jj['addons_service_qty'];
            }
        }
    }
	if($company_name == ""){
	...
	$bid = $orderid;
  $hid = md5('hashedpassword'.$bid);
  if ($service_name == "The Order Of The 9") $eheading = '<br>We have a big problem. This case is far more complicated than we thought. We don\'t have any leads, and time is running out!<br><br>We are delighted to hear that you and your team of expert detectives will be coming to help with the mystery of The Order Of The 9, we desperately need the assistance!<br><br>We have been doing our absolute best to investigate and solve the murders, but just can\'t crack the case. We\'re hoping that you\'ll be able to solve the mystery before it\'s too late!<br><br>IMPORTANT: We need to ensure all detectives are protected! For this reason please send the following to your team members to complete their indemnity form before arrival to your adventure. The form can be <a href="https://www.mindgame.co.za/waiver/waiver.php?booking='.$bid.'&hash='.$hid.'">found here</a>.<br><br>We cannot wait to see you on the '.$booking_date.' at '.$booking_time.'. We will be expecting '.$detectives.' detectives at the following address: 2nd Floor, Unit 205, Granada Square, 16 Chartwell Drive, Umhlanga, Durban.<br><br>Inside intelligence indicates that the The Order Of The 9 are aware that expert detectives are on route, and may try to delay you. For this reason, please make sure you arrive 15 minutes before your game start time, so we can give you the low-down and to ensure the investigation starts on time. If you are late, it sadly may mean that your investigation time is limited. You will find parking for your horse and carriage on the side of the building, with entrances in both Ocean Way and Lagoon Drive. From there you can take the strange metal box up to level 2 (we are unsure as to how this contraption works) and we are the first entrance to your right.<br><br>If you opt to arrive on foot instead of horse and carriage, the lifts are next to Woolworths. Woolworths is just behind Little Havana\'s on Chartwell Drive. <br><br>We will be waiting in great anticipation! <br><br>Mysteriously yours,<br><br>The team at Mindgame Escape';
  if ($service_name == "Bunker 51") $eheading = '<br>We have a big problem: The mystery of Bunker 51 has still not been solved.<br><br>We are delighted to hear that you and your team of expert detectives will be coming to help with the mystery of Bunker 51, we desperately need the assistance!<br><br>While we have been doing our absolute best to investigate and solve the mystery, we have still not been able to crack the case. We\'ll hand the case over to you and we are hoping you will be able to solve it before it\'s too late!<br><br>IMPORTANT: We need to ensure all detectives are protected! For this reason please send the following to your team members to complete their indemnity form before arrival to your adventure. The form can be <a href="https://www.mindgame.co.za/waiver/waiver.php?booking='.$bid.'&hash='.$hid.'">found here</a>.<br><br>We cannot wait to see you on the '.$booking_date.' at '.$booking_time.'. We will be expecting '.$detectives.' detectives at the following address: 2nd Floor, Unit 205, Granada Square, 16 Chartwell Drive, Umhlanga, Durban.<br><br>Inside intelligence indicates that Bunker 51 spies may be aware that expert detectives are on route, and may try to delay you. For this reason, please make sure you arrive 15 minutes before your game start time, so we can give you the low-down and to ensure the investigation starts on time. If you are late, it sadly may mean that your investigation time is limited. <br><br>You will find parking for your horse and carriage on the side of the building, with entrances in both Ocean Way and Lagoon Drive. From there you can take the strange metal box up to level 2 (we are unsure as to how this contraption works) and we are the first entrance to your right.<br><br>If you opt to arrive on foot instead of horse and carriage, the lifts are next to Woolworths. Woolworths is just behind Little Havana\'s on Chartwell Drive. <br><br>We will be waiting in great anticipation - Bunker 51 is counting down, and will be locked forever if we cannot solve the mystery in time!<br><br>Mysteriously yours,<br><br>The team at Mindgame Escape';
  if ($service_name == "Forgotten Temple") $eheading = '<br>We have a big problem: The curse of the Forgotten Temple is real!<br><br>We are delighted to hear that you and your team of expert archaeologists will be coming to assist with the curse of the Forgotten Temple, we desperately need the assistance!<br><br>While we have been doing our absolute best to investigate and solve the mystery, we have still not been able to break the curse. Our last team of explorers never returned, so we will hand the case over to you and we are hoping you will be able to solve it before it\'s too late!<br><br>IMPORTANT: We need to ensure all detectives are protected! For this reason please send the following to your team members to complete their indemnity form before arrival to your adventure. The form can be <a href="https://www.mindgame.co.za/waiver/waiver.php?booking='.$bid.'&hash='.$hid.'">found here</a>.<br><br>We cannot wait to see you on the '.$booking_date.' at '.$booking_time.'. We will be expecting '.$detectives.' explorers at the following address: 2nd Floor, Unit 205, Granada Square, 16 Chartwell Drive, Umhlanga, Durban.<br><br>Inside intelligence indicates that the Mayan King may be aware that expert archaeologists are on route, and may try to delay you. For this reason, please make sure you arrive 15 minutes before your adventure start time, so we can give you the low-down and to ensure the exploration starts on time. If you are late, it sadly may mean that your exploration time is limited. <br><br>You will find parking for your horse and carriage on the side of the building, with entrances in both Ocean Way and Lagoon Drive. From there you can take the strange metal box up to level 2 (we are unsure as to how this contraption works) and we are the first entrance to your right.<br><br>If you opt to arrive on foot instead of horse and carriage, the lifts are next to Woolworths. Woolworths is just behind Little Havana\'s on Chartwell Drive. <br><br>We will be waiting in great anticipation - the world needs you to break the curse before it\'s too late!<br><br>Mysteriously yours,<br><br>The team at Mindgame Escape';
  if ($service_name == "Escape to Treasure Island") {
        $eheading = '<br>Attention, thrill seekers and puzzle enthusiasts! Prepare yourselves for an electrifying experience at the Pavilion Mall.<br><br>Get ready to unleash your inner detective and embark on an extraordinary journey like no other! We have some thrilling top-secret intel to share.<br><br>Location Reconnaissance:<br>Arrive the Pavilion Entrance 12 Rooftop Parking, right next to the Olive and Oil Restaurant. (Check out our Instagram or Facebook for detailed parking directions)<br><br>Fuel for the Brain:<br>A critical mission requires optimal brainpower! Upon arrival, chat with your friendly Gamemaster as we\'ve partnered with none other than Afros Chicken. Place your order with the gamemaster, and a delectable feast will be awaiting you once your adventure as come to an end. Choose from a variety of yummy options, including Halaal.<br><br>Bonus Missions Await:<br>The excitement doesn\'t end with your escape mission! Engage in extra fun at the Battle Zone, offering epic games like Nerf wars, Jelly ball, and target shooting. Elevate your adventure with these adrenaline-pumping activities!<br><br>IMPORTANT: We need to ensure all detectives are protected! For this reason please send the following to your team members to complete their indemnity form before arrival to your adventure. The form can be <a href="https://www.mindgame.co.za/waiver/waiver.php?booking='.$bid.'&hash='.$hid.'">found here</a>.<br><br>We cannot wait to see you on the '.$booking_date.' at '.$booking_time.'. We will be expecting '.$detectives.' explorers at the following address: Pavillion Mall, Westville.<br><br>We will be waiting in great anticipation - the world needs you to break the curse before it\'s too late!<br><br>Mysteriously yours,<br><br>The team at Mindgame Escape';
    }
  if ($service_name == "The Great Time Escape") {
        $eheading = '<br>Attention, thrill seekers and puzzle enthusiasts! Prepare yourselves for an electrifying experience at the Pavilion Mall.<br><br>Get ready to unleash your inner detective and embark on an extraordinary journey like no other! We have some thrilling top-secret intel to share.<br><br>Location Reconnaissance:<br>Arrive the Pavilion Entrance 12 Rooftop Parking, right next to the Olive and Oil Restaurant. (Check out our Instagram or Facebook for detailed parking directions)<br><br>Fuel for the Brain:<br>A critical mission requires optimal brainpower! Upon arrival, chat with your friendly Gamemaster as we\'ve partnered with none other than Afros Chicken. Place your order with the gamemaster, and a delectable feast will be awaiting you once your adventure as come to an end. Choose from a variety of yummy options, including Halaal.<br><br>Bonus Missions Await:<br>The excitement doesn\'t end with your escape mission! Engage in extra fun at the Battle Zone, offering epic games like Nerf wars, Jelly ball, and target shooting. Elevate your adventure with these adrenaline-pumping activities!<br><br>IMPORTANT: We need to ensure all detectives are protected! For this reason please send the following to your team members to complete their indemnity form before arrival to your adventure. The form can be <a href="https://www.mindgame.co.za/waiver/waiver.php?booking='.$bid.'&hash='.$hid.'">found here</a>.<br><br>We cannot wait to see you on the '.$booking_date.' at '.$booking_time.'. We will be expecting '.$detectives.' explorers at the following address: Pavillion Mall, Westville.<br><br>We will be waiting in great anticipation - the world needs you to break the curse before it\'s too late!<br><br>Mysteriously yours,<br><br>The team at Mindgame Escape';
    }
  ...
  $searcharray = array('{{whichroom}}','{{service_name}}','{{booking_date}}','{{business_logo}}','{{business_logo_alt}}','{{client_name}}','{{methodname}}','{{units}}','{{addons}}','{{firstname}}','{{lastname}}','{{client_email}}','{{phone}}','{{payment_method}}','{{vaccum_cleaner_status}}','{{parking_status}}','{{notes}}','{{contact_status}}','{{admin_name}}','{{price}}','{{address}}','{{app_remain_time}}','{{reject_status}}','{{company_name}}','{{booking_time}}','{{client_city}}','{{client_state}}','{{client_zip}}','{{company_city}}','{{company_state}}','{{company_zip}}','{{company_country}}','{{company_phone}}','{{company_email}}','{{company_address}}','{{admin_name}}');
    ...
    if($settings->get_option('ct_admin_email_notification_status') == 'Y' && $adminemailtemplate['email_template_status'] == 'E'){							
        ...
        $smses = 'New Booking: '.$client_name . ' : ' . $service_name.' - ' .$booking_date . ' @ ' . $booking_time .' - '.$detectives.' players';
				$bodies = urlencode($smses);
				$gogo = curl_init("http://api.clickatell.com/http/sendmsg?user=NGR0011&password=YWXfYUOfObSCPM&api_id=3620961&to=27829783824&text=".$bodies);
				curl_setopt($gogo, CURLOPT_POST, true);
				curl_setopt($gogo, CURLOPT_POSTFIELDS, $data);
				curl_setopt($gogo, CURLOPT_RETURNTRANSFER, true);
				curl_exec($gogo);
				$gogo = curl_init("http://api.clickatell.com/http/sendmsg?user=NGR0011&password=YWXfYUOfObSCPM&api_id=3620961&to=27769348437&text=".$bodies);
				curl_setopt($gogo, CURLOPT_POST, true);
				curl_setopt($gogo, CURLOPT_POSTFIELDS, $data);
				curl_setopt($gogo, CURLOPT_RETURNTRANSFER, true);
				curl_exec($gogo);
				$gogo = curl_init("http://api.clickatell.com/http/sendmsg?user=NGR0011&password=YWXfYUOfObSCPM&api_id=3620961&to=27847347938&text=".$bodies);
				curl_setopt($gogo, CURLOPT_POST, true);
				curl_setopt($gogo, CURLOPT_POSTFIELDS, $data);
				curl_setopt($gogo, CURLOPT_RETURNTRANSFER, true);
				curl_exec($gogo);
				$gogo = curl_init("http://api.clickatell.com/http/sendmsg?user=NGR0011&password=YWXfYUOfObSCPM&api_id=3620961&to=27713833685&text=".$bodies);
				curl_setopt($gogo, CURLOPT_POST, true);
				curl_setopt($gogo, CURLOPT_POSTFIELDS, $data);
				curl_setopt($gogo, CURLOPT_RETURNTRANSFER, true);
				curl_exec($gogo);
    ...
    if(isset($_SESSION['ct_details']['payment_method']) && ($_SESSION['ct_details']['payment_method']=='paypal')){
		...
		<script>window.location = '<?php echo $thankyou_page_url; ?>'; </script>
		<?php
    } else if(isset($_SESSION['ct_details']['payment_method']) && ($_SESSION['ct_details']['payment_method']=='payumoney2')){
        if($settings->get_option('ct_thankyou_page_url') == ''){
            $thankyou_page_url = SITE_URL.'front/thankyou.php';
        }else{
            $thankyou_page_url = $settings->get_option('ct_thankyou_page_url');
