<?php
	...
	while($e_reminder = mysqli_fetch_array($book_details)){
		...
		if ($service_name == "Escape to Treasure Island") $address_new = ' at the following address: Pavillion Mall, Westville';
		if ($service_name == "The Great Time Escape") $address_new = ' at the following address: Pavillion Mall, Westville';
		...
		if($email_reminder_time >= $time_in_min && $binfo[12]!=1){
			...
			if($setting->get_option('ct_admin_email_notification_status')=='Y' && $adminemailtemplate[4]=='E'){
				...
				$adminsubject= "REMINDER: " . ucwords($service_name)." on ".$booking_date;
				$mail_a->Subject = $adminsubject;
				$mail_a->Body = $admin_email_body;
				$mail_a->send();*/
				$sms = 'REMINDER: '.$client_name . ' : ' . $service_name.' - ' .$booking_date . ' @ ' . $booking_time .' - '.$detectives.' players';
				$body = urlencode($sms);
