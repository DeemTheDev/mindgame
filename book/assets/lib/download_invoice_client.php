...
ob_start();	session_start();
	...
	include(dirname(dirname(dirname(__FILE__))).'/assets/pdf/fpdf.php');
...
else
{
	...
	{ 
		$payment_method = utf8_decode($label_language_values['cash']); 
	}			
	else if($payinfo[2]=='Card Payment')
	{
		$payment_method = utf8_decode($label_language_values['card_payment']); 
	}	
	...
	if($business_logo!=='' || $business_logo!==null){
		$logo="../images/services/".$business_logo;
	}else{
	...
	if($client_city != '' && $client_state != ''){
		$client_city_state = utf8_decode($client_city).",".utf8_decode($client_state);
	}elseif($client_city != '' && $client_state == ''){
		$client_city_state = utf8_decode($client_city);
	}elseif($client_city == '' && $client_state != ''){
		$client_city_state = utf8_decode($client_state);
	}
	...
	$pdf->Image('../images/background_image_client.jpeg',0,0,210); /* background */
	$pdf->Image('../images/services/company_86714.jpg',10,0,40); //  Logo 
	$pdf->SetFont('Arial','',12);
	$pdf->Text(133,10,utf8_decode($business_name));
	$pdf->Text(133,15,utf8_decode('Unit 205, 2nd Floor Granada Square'));
	$pdf->Text(133,20,utf8_decode('16 Chartwell Drive, Umhlanga'));
	$pdf->Text(133,25,utf8_decode($business_city));
	//$pdf->Text(25,25,utf8_decode($business_country));
	$pdf->Text(140,39,utf8_decode($business_email));
	$pdf->Text(140,33,utf8_decode($client_phone));
	$pdf->Text(133,46,utf8_decode('VAT No: 4280285125'));
	
	$pdf->SetFont('Arial','',13);
	//$pdf->Text(133,10,utf8_decode($label_language_values['invoice_to']));
	
	$pdf->SetFont('Arial','',10);
	$pdf->Text(133,15,utf8_decode(ucwords($client_name)));
	
	$pdf->SetFont('Arial','',9);
	
	/* here first no.is position from left and second is from top ok */
	$pdf->Text(133,20,utf8_decode($client_address));
	$pdf->Text(133,25,$client_city_state);
	
	$pdf->Text(140,38,utf8_decode($client_email));
	
	$pdf->SetFont('Arial','',20);
	$pdf->SetTextColor(55,55,55);
	$pdf->Text(30,60,utf8_decode('VAT '.$label_language_values['invoice']));
	$pdf->SetFont('Arial','',22);
	$pdf->Text(31,75,"#".sprintf("%04d",$order_id));

	$pdf->SetFont('Arial','',13);
	$pdf->SetTextColor(255,255,255);
	$pdf->Text(98,60 ,utf8_decode($label_language_values['invoice_date']));
	...
	$pdf->Text(160,60,utf8_decode($label_language_values['payment_method']));
	...
	if($payment_method == 'Bank Transfer'){
		$pdf->Text(173-($pdf->GetStringWidth(utf8_decode($payment_method))/2),68,strtoupper($payment_method));
	}else{
		$pdf->Text(177-($pdf->GetStringWidth(utf8_decode($payment_method))/2),68,strtoupper($payment_method));
	}
	/* $pdf->Text(181 - $pdf->GetStringWidth(strtoupper($payment_method))/2,82,strtoupper($payment_method)); */
	
	$pdf->SetFont('Arial','',13);
	$pdf->SetTextColor(55,55,55);
	$pdf->Text(20,107,utf8_decode($label_language_values['service_name']));
	/* $pdf->Text(50,107,"Method"); */
	/* $pdf->Text(100,107,"Unit"); */
	$pdf->Text(135,107,utf8_decode($label_language_values['qty']));
	/* $pdf->Text(150,107,""); */
	$pdf->Text(179,107,utf8_decode($label_language_values['price']));
	
	
	
	$addondetails_startpoint = 122;
	$pdf->SetFont('Arial','',11);
	$pdf->Text(20,118,utf8_decode($booking_info_details[0]['service_name']));
	$pdf->SetFont('Arial','',9);
	foreach($booking_info_details as $book_detail){	
		if($book_detail['unitname'] != "")
		{		
			$pdf->Text(22,$addondetails_startpoint,utf8_decode($book_detail['unitname']));
			$pdf->Text(137,$addondetails_startpoint,utf8_decode($book_detail['methodqty']));
			/* $pdf->Text(150,$addondetails_startpoint,$book_detail['service_price']); */
			$pdf->SetFont('Arial','',10);
			$pdf->Text(186-$pdf->GetStringWidth('R'.number_format(($payment_amount/1.15)),2,'.',''),$addondetails_startpoint,'R'.number_format(($payment_amount/1.15),2,'.',''));
	...
	foreach($booking_addon_details as $booking_addon){	
			...
			$pdf->Text(22,$addondetails_sttpoint,utf8_decode($booking_addon['addonname']));
			$pdf->Text(137,$addondetails_sttpoint,utf8_decode($booking_addon['addonqty']));
	...
	$pdf->Text(155-$pdf->GetStringWidth(utf8_decode($label_language_values['sub_total'])),$addondetails_sttpoint+5,utf8_decode($label_language_values['sub_total']));
	if($payinfo[11] == 'O'){
    $fd = "Once";
		}
		elseif($payinfo[11] == 'W'){
	...
	//$pdf->Text(155-$pdf->GetStringWidth(utf8_decode($label_language_values['frequently_discount'])."(".$fd.")"),$addondetails_sttpoint+10,utf8_decode($label_language_values['frequently_discount'])."(".$fd.")");
	if ($general->ct_price_format_for_pdf($payment_discount,$symbol_position,$decimal) <> 0.00) $pdf->Text(155-$pdf->GetStringWidth(utf8_decode($label_language_values['coupon_discount'])),$addondetails_sttpoint+15,utf8_decode($label_language_values['coupon_discount']));
	$pdf->Text(155-$pdf->GetStringWidth(utf8_decode('VAT @ 15%')),$addondetails_sttpoint+20,utf8_decode('VAT @ 15%'));
	
	 $printamount=$general->ct_price_format_for_pdf(($payment_amount/1.15),$symbol_position,$decimal); 
	 $printtaxes=$general->ct_price_format_for_pdf((($payment_amount/1.15)*0.15),$symbol_position,$decimal);	  
	 $printdiscount='-'.$general->ct_price_format_for_pdf(($payment_amount/1.15),$symbol_position,$decimal);	   
	 //$printfrequency='-'.$general->ct_price_format_for_pdf($payinfo[12],$symbol_position,$decimal);	   
	 $printnetamount=$general->ct_price_format_for_pdf(($payment_amount/1.15),$symbol_position,$decimal);
	 $printnetamount_withVat=$general->ct_price_format_for_pdf(($payment_net_amount),$symbol_position,$decimal);
	...
	if ($general->ct_price_format_for_pdf($payment_discount,$symbol_position,$decimal) <> 0.00) $pdf->Text(190-$pdf->GetStringWidth($printdiscount),$addondetails_sttpoint+15,$printdiscount);
	$pdf->Text(190-$pdf->GetStringWidth($printtaxes),$addondetails_sttpoint+20,$printtaxes);
	
	$pdf->SetFont('Arial','',13);
	$pdf->SetTextColor(255,255,255);
	$pdf->Text(155-$pdf->GetStringWidth(utf8_decode($label_language_values['total'].' incl VAT')),265,utf8_decode($label_language_values['total'].' incl VAT'));
	$pdf->Text(185-$pdf->GetStringWidth($printtaxes),265,$printnetamount_withVat);
...
/*	$pdf->Text(23,217,"Payment Information");
	...
	$label_language_values['booked_on'] = 'Booked For';
	$pdf->SetFont('Arial','',12);
	$pdf->Text(15,265,utf8_decode($label_language_values['booked_on']));
	...
	$pdf->Text($pdf->GetStringWidth(($label_language_values['booked_on'].' '))+15,265,date("d/m/Y",strtotime($bookings[1])));
	$pdf->Text($pdf->GetStringWidth(($label_language_values['booked_on']))+ $pdf->GetStringWidth(($datevar)) + 18,265,$book_times);
		...
		$pdf->SetFont('Arial','',10);
		$pdf->Text(17,290,"Standard Bank, Name: Mindgame Escape, Account: 302036520, Branch: Durban North, Type: Cheque Account"); 
		$pdf->Text(77,280,"This is a computer generated VAT invoice"); 
