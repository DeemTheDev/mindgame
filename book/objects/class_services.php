...
class cleanto_services{
    ...
    public function get_used_images(){
        $query = "select s.image as image from ct_services as s
UNION
select sa.image as image from ct_services_addon as sa
UNION
select setim.option_value as setimage from ct_settings as setim where option_name = 'ct_company_logo'";
        $result=mysqli_query($this->conn,$query);
        return $result;
    }
	/*  TO GET ALL IMAGES NAMES FROM SERVICE,ADDONS TABLE FOR DELETING NOT USED IN DIRECTORY */
	...
	public function readall_for_frontend_services(){
		...
		//$query="(select ct_services.* from ct_services,ct_services_method,ct_service_methods_units where ct_services.status = 'E' and ((ct_services_method.status = 'E' and ct_service_methods_units.status = 'E' and ct_services_method.service_id = ct_services.id and ct_service_methods_units.services_id = ct_services.id)) group by ct_services.id ORDER BY position) UNION (select ct_services.* from ct_services,ct_services_addon where ct_services.status = 'E' and ct_services_addon.status = 'E' and ct_services_addon.service_id = ct_services.id group by ct_services.id) ORDER BY position"; 
		$query="(select ct_services.* from ct_services,ct_services_method,ct_service_methods_units where ct_services.status = 'E' and ct_services.id <= 3 and ((ct_services_method.status = 'E' and ct_service_methods_units.status = 'E' and ct_services_method.service_id = ct_services.id and ct_service_methods_units.services_id = ct_services.id)) group by ct_services.id ORDER BY position) UNION (select ct_services.* from ct_services,ct_services_addon where ct_services.status = 'E' and ct_services_addon.status = 'E' and ct_services_addon.service_id = ct_services.id group by ct_services.id) ORDER BY position";
		$result=mysqli_query($this->conn,$query);
		return $result;
 }

 public function readall_for_frontend_services_corn(){
		
    $query="(select ct_services.* from ct_services,ct_services_method,ct_service_methods_units where ct_services.status = 'E' and ct_services.id > 3 and ((ct_services_method.status = 'E' and ct_service_methods_units.status = 'E' and ct_services_method.service_id = ct_services.id and ct_service_methods_units.services_id = ct_services.id)) group by ct_services.id ORDER BY position) UNION (select ct_services.* from ct_services,ct_services_addon where ct_services.status = 'E' and ct_services.id > 3 and ct_services_addon.status = 'E' and ct_services_addon.service_id = ct_services.id group by ct_services.id) ORDER BY position";
