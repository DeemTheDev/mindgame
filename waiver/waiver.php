...
$hash = $_GET['hash'];
if (md5('hashedpassword'.$booking_number) == $hash) {

if ($_POST['booking'] && $_POST['lastName'] && $_POST['email']) {
	$player_template = mysqli_query($db, "select * from ct_order_client_players where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
		$player = mysqli_fetch_assoc($player_template);
		if ($player['order_id'] >= 1) {
			if ($player['player2'] == "") {
				mysqli_query($db, "update ct_order_client_players set player2 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player3'] == "") {
				mysqli_query($db, "update ct_order_client_players set player3 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player4'] == "") {
				mysqli_query($db, "update ct_order_client_players set player4 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player5'] == "") {
				mysqli_query($db, "update ct_order_client_players set player5 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player6'] == "") {
				mysqli_query($db, "update ct_order_client_players set player6 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player7'] == "") {
				mysqli_query($db, "update ct_order_client_players set player7 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			} else if ($player['player8'] == "") {
				mysqli_query($db, "update ct_order_client_players set player8 = '".mysqli_real_escape_string($db, $_POST['email'])."' where order_id = '".mysqli_real_escape_string($db, $_POST['booking'])."'");
			}
			//echo '<br>Record Exists '.$_POST['booking'];	
		} else {
			mysqli_query($db, "insert into ct_order_client_players (order_id, player1) values ('".mysqli_real_escape_string($db, $_POST['booking'])."','".mysqli_real_escape_string($db, $_POST['email'])."')"); 
  ...
  <div class="container">
	...
	<form class="registration-form" method="post">
		<label>
			...
			<input type="hidden" name="booking" value="<?=$booking_number;?>">
			<input type="hidden" name="hash" value="<?=$hash;?>">
			<? 
				 $products_query_template = 'select ci.order_id, client_name, booking_date_time, title from ct_services, ct_bookings cb, ct_order_client_info ci where ct_services.id = service_id and cb.order_id = ci.order_id and cb.booking_status="A" and ci.order_id = ' . $booking_number;
				 $bookings_query = mysqli_query($db, $products_query_template);
				 while ($bookings = mysqli_fetch_assoc($bookings_query)) {
				 	echo '<div>'.$bookings['client_name'] . ' at ' . substr($bookings['booking_date_time'],11) . ' in ' . $bookings['title'].'</div>';
				 } ?>
