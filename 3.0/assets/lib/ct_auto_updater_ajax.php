...
if (extension_loaded('zip')) {
	...
	$server_path = "http://skymoonlabs.com/cleanto/";
	...
	{
		$getVersions = get_contents('http://skymoonlabs.com/cleanto/versioncheck.php?'.time());
		if ($getVersions != '')
		{
			...
			foreach ($versionList as $aV)
			{
				if ( $aV > $settings->get_option('ct_version')) 
				{
					...
					if ( !is_file(dirname(dirname(dirname(__FILE__))).'/ct-updates/ct-'.$aV.'.zip' )) 
					{
						...
						$newUpdate = url_get_contents($server_path.'ct-updates/ct-'.$aV.'.zip');
