...
class cleanto_dummy{
	...
	public function add_services_method_unit(){
		$query="insert into `".$this->service_methods_units."` (`id`,`services_id`,`methods_id`,`units_title`,`base_price`,`maxlimit`,`status`, `position`) values(NULL,'".$this->smu_service_id."','".$this->smu_methods_id."','".$this->smu_units_title."','".$this->smu_base_price."','".$this->smu_maxlimit."','".$this->smu_status."', '0')";
		$result=mysqli_query($this->conn,$query);
		$value=mysqli_insert_id($this->conn);
		return $value;
	}
	...
	public function add_off_days(){
		$query="insert into `".$this->off_days."` (`id`,`user_id`,`off_date`,`lastmodify`,`status`) values(NULL,'0','".$this->od_off_date."','".$this->od_lastmodify."','0')";
		$result=mysqli_query($this->conn,$query);		
		$value=mysqli_insert_id($this->conn);
		return $value;
	}
	...
	public function add_settings(){
	 ...
	 public function get_all_bookings_byserviceid($id){
        $query="select `order_id` from `".$this->bookings."` where `service_id` = '".$id."' GROUP BY `id`, `order_id`, `client_id`, `order_date`, `booking_date_time`, `service_id`, `method_id`, `method_unit_id`, `method_unit_qty`, `method_unit_qty_rate`, `booking_status`, `reject_reason`, `reminder_status`, `lastmodify`, `read_status`, `staff_ids`";
        $result=mysqli_query($this->conn,$query);
        return $result;
    }
