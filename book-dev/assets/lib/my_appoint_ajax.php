            ...
            <div class="modal-content">
                ...
                <div class="modal-body mb-20">
                    <ul class="list-unstyled ct-cal-booking-details mypopupul">
                        <li>
                            <label style="width: 120px;"><?php echo $label_language_values['booking_status'];?> : </label>
				...
				<?php } ?>		
						...
						<hr>
						<li>
							<label><?php echo $label_language_values['assign_appointment_to_staff'];?></label>
                ...
                <div class="modal-footer">
                    <div class="cta-col12 ct-footer-popup-btn">
                        ...
                        if($orderdetail[6]=='C' || $orderdetail[6]=='R' || $orderdetail[6]=='CC' || $past_day == "yes"){
							...
							<span class="col-md-3 col-sm-3 col-xs-4 np ct-w-32"></span>
                            <span class="col-md-2 col-sm-2 col-xs-4 np ct-w-32">
...
elseif(isset($_POST['confirm_eft'])){
 $id = $_POST['id'];
 $method = 'EFT';
 $objdashboard->confirm_payment($id,$method);
}
elseif(isset($_POST['confirm_cc'])){
 $id = $_POST['id'];
 $method = 'Credit Card';
 $objdashboard->confirm_payment($id,$method);
}
...
elseif(isset($_POST['confirm_booking_cal'])){
    ...
    $whichroom = '<br>Thank you very much for assisting with the '.$service_name.' case - you and your team were amazing!<br><br>We have one last favour to ask from you? We are receiving reports that the general public are still unsettled, and need confirmation that your team of expert detectives assisted with the case! If you could PLEASE help put their minds at ease so they can sleep better at night, it would be fantastic. Our gamemasters also receive a little something for each and every review, so you are doing a good deed for them and boosting morale too!<br><br>We would love if you would please review on as many platforms as possible; we love reviews on <a href="https://www.facebook.com/pg/MindgameEscape/reviews/?ref=page_internal">Facebook</a>, <a href="https://www.tripadvisor.co.za/Attraction_Review-g10679656-d12827635-Reviews-Mindgame_Escape-Umhlanga_KwaZulu_Natal.html">Tripadvisor</a>, <a href="https://search.google.com/local/writereview?placeid=ChIJx5Q39FkP9x4RZkJpxBXmdQw">Google</a>. The pictures that were taken of your adventure will be loaded on <a href="https://www.tripadvisor.co.za/Attraction_Review-g10679656-d12827635-Reviews-Mindgame_Escape-Umhlanga_KwaZulu_Natal.html">Tripadvisor</a>, <a href="https://www.facebook.com/MindgameEscape/">Facebook</a>, <a href="https://www.instagram.com/mindgameescape/">Instagram</a>, <a href="https://www.linkedin.com/company/18214668/">Linkedln</a>, <a href="https://plus.google.com/116195037511663654261">Google+</a> and <a href="https://twitter.com/MindgameEscape">Twitter</a> - so please keep a lookout for them and share with the world!<br><br>We wish you all the best with your future investigations, and hope to see you again soon!<br><br>Mysteriously yours,<br><br>Mindgame Escape<br><br>P.S If your experience for any reason was not phenominal, please let the owner know on nick@mindgame.co.za so we can investigate and make things right.';
    
    $searcharray = array('{{whichroom}}','{{service_name}}','{{booking_date}}','{{business_logo}}','{{business_logo_alt}}','{{client_name}}','{{methodname}}','{{units}}','{{addons}}','{{client_email}}','{{phone}}','{{payment_method}}','{{vaccum_cleaner_status}}','{{parking_status}}','{{notes}}','{{contact_status}}','{{address}}','{{price}}','{{admin_name}}','{{firstname}}','{{lastname}}','{{app_remain_time}}','{{reject_status}}','{{company_name}}','{{booking_time}}','{{client_city}}','{{client_state}}','{{client_zip}}','{{company_city}}','{{company_state}}','{{company_zip}}','{{company_country}}','{{company_phone}}','{{company_email}}','{{company_address}}','{{admin_name}}');

    $replacearray = array($whichroom, $service_name, $booking_date , $business_logo, $business_logo_alt, $client_name,$methodname, $units, $addons,$client_email, $client_phone, $payment_status, $final_vc_status, $final_p_status, $client_notes, $client_status,$client_address,$price,$get_admin_name,$firstname,$lastname,'','',$admin_company_name,$booking_time,$client_city,$client_state,$client_zip,$company_city,$company_state,$company_zip,$company_country,$company_phone,$company_email,$company_address,$get_admin_name);
    ...
    if($setting->get_option('ct_client_email_notification_status') == 'Y' && $clientemailtemplate[4]=='E' ){
        ...
        $player_query="select * from ct_order_client_players where order_id = '".$id."'";
  			$players = mysqli_fetch_array(mysqli_query($objdashboard->conn,$player_query));
				if ($players['player1'] <> '') $mail->addCustomHeader("BCC: ".$players['player1']);
				if ($players['player2'] <> '') $mail->addCustomHeader("BCC: ".$players['player2']);
				if ($players['player3'] <> '') $mail->addCustomHeader("BCC: ".$players['player3']);
				if ($players['player4'] <> '') $mail->addCustomHeader("BCC: ".$players['player4']);
				if ($players['player5'] <> '') $mail->addCustomHeader("BCC: ".$players['player5']);
				if ($players['player6'] <> '') $mail->addCustomHeader("BCC: ".$players['player6']);
				if ($players['player7'] <> '') $mail->addCustomHeader("BCC: ".$players['player7']);
				if ($players['player8'] <> '') $mail->addCustomHeader("BCC: ".$players['player8']);
