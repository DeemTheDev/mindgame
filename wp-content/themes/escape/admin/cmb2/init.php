...
if ( ! class_exists( 'CMB2_Bootstrap_221', false ) ) {
	...
	class CMB2_Bootstrap_221 {
		...
		public function include_cmb() {
			...
			if ( ! defined( 'CMB2_DIR' ) ) {
				define( 'CMB2_DIR', trailingslashit( get_template_directory() .'/admin/cmb2/' ) );
			}
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'escape', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'escape', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'escape', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'escape' );
				$mofile = get_template_directory() . '/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'escape', $mofile );
			}
